...
public class CraftingRecipeProcess<R extends TLRecipe,S extends IMultiblockState> extends TLRecipeProcess<R,S> {
    ...
    public CraftingRecipeProcess(Class<R> type, List<Action<R, S>> actions, S state, BiFunction<TLRecipeProcess<R, S>, Integer, Boolean> tickLogic, TLRecipeSerializer<R> serializer) {
        super(type, actions, state, tickLogic);
        this.serializer = serializer;
    }

    public CraftingRecipeProcess(Class<R> type, List<Action<R, S>> actions, S state, BiFunction<TLRecipeProcess<R, S>, Integer, Boolean> tickLogic, int initialTick, TLRecipeSerializer<R> serializer) {
        super(type, actions, state, tickLogic, initialTick);
        this.serializer = serializer;
    }

    public CraftingRecipeProcess(Class<R> type, List<Action<R, S>> actions, S state, BiFunction<TLRecipeProcess<R, S>, Integer, Boolean> tickLogic, int[] initialTick, int maxParallel, boolean allowDifferentRecipes, TLRecipeSerializer<R> serializer) {
        super(type, actions, state, tickLogic, initialTick, maxParallel, allowDifferentRecipes);
        this.serializer = serializer;
    }

    public CraftingRecipeProcess(Class<R> type, List<Action<R, S>> actions, S state, BiFunction<TLRecipeProcess<R, S>, Integer, Boolean> tickLogic, int maxParallel, boolean allowDifferentRecipes, TLRecipeSerializer<R> serializer) {
